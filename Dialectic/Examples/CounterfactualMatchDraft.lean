...
/-!
An editing-state version of `CounterfactualMatch.lean`.

Place the cursor on the bare `Step` below. Dialectic treats it as a drafting
marker, shows the moves supported by the Counterfactual profile, and offers a
standard VS Code Quick Fix that inserts an explicit Goal and complete Step.
-/

Argument CounterfactualMatchDraft
